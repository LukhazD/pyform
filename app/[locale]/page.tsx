...
const DeckController = (): null => {
  useEffect(() => {
    ...
    const initGsap = async () => {
      ...
      // Wait until all deck elements exist in the DOM before creating ScrollTriggers.
      // Dynamic imports may not have rendered yet on locale switch.
      const waitForElements = () => {
        const hero = document.getElementById("deck-hero");
        const problem = document.getElementById("deck-problem");
        const features = document.getElementById("deck-features");

        if (!hero || !problem || !features) {
          rafId = requestAnimationFrame(waitForElements);
          return;
        }
        ...
        // Force recalculation after all elements are measured
